  ...
  return (
    <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
      <div className="my-7 w-full">
        <img src={user.imageUrl} alt="profilepic" className='w-13 rounded-full mx-auto' />
        ...
        <div>
          {navItems.map(({ to, label, Icon: IconComp }) => (
            <NavLink
              ...
              end={to === '/ai'} // ✅ fix for dashboard active highlighting
              className={({ isActive }) =>
            ...
            >
              {({ isActive }) => (
                <>
                  <IconComp className={`w-4 h-4 ${isActive ? 'text-white' : ''}`} />
                  {label}
                </>
              )}
            </NavLink>
